...
std::optional<odb::Point> Opendp::placeAt(odb::dbInst* db_inst,
                                          const odb::Point& requested_loc)
{
  Node* cell = network_->getNode(db_inst);
  if (cell == nullptr || cell->isFixed()) {
    return std::nullopt;
  }

  const GridX orig_x = grid_->gridX(cell);
  const GridY orig_y = grid_->gridSnapDownY(cell);
  const bool was_placed = cell->isPlaced();

  // Refuse the move if the current location can't be restored. placeCell()
  // calls getSiteOrientation(x,y,site).value() unconditionally, which crashes
  // when the cell sits at coordinates whose row_sites_ has no entry for the
  // cell's master site (e.g., off-row from upstream placement, or a multi-row
  // master whose site doesn't match the bottom row at orig_y).
  if (was_placed) {
    odb::dbSite* site = cell->getDbInst()->getMaster()->getSite();
    if (!grid_->getSiteOrientation(orig_x, orig_y, site)) {
      return std::nullopt;
    }
  }

  // Free the source pixel so diamondSearch can pick it (or a neighbor) and
  // canBePlaced doesn't see this cell colliding with itself.
  if (was_placed) {
    unplaceCell(cell);
  }

  // Snap requested loc into core-relative grid coords, mirroring the
  // legalCellPos pre-snap (off-die / off-macro) so we don't ask diamondSearch
  // for a point outside the search domain.
  const DbuPt core_relative{DbuX{requested_loc.x() - core_.xMin()},
                            DbuY{requested_loc.y() - core_.yMin()}};
  const DbuPt snapped = legalPt(cell, core_relative);
  const GridPt req_pt{grid_->gridX(snapped.x), grid_->gridSnapDownY(snapped.y)};

  const PixelPt found = diamondSearch(cell, req_pt.x, req_pt.y);
  if (found.pixel == nullptr) {
    if (was_placed) {
      placeCell(cell, orig_x, orig_y);
    }
    debugPrint(logger_,
               DPL,
               "place",
               1,
               "placeAt {} ({},{}): no free pixel within search bounds",
               db_inst->getName(),
               requested_loc.x(),
               requested_loc.y());
    return std::nullopt;
  ...
  placeCell(cell, found.x, found.y);
  const odb::Point final_loc(core_.xMin() + cell->getLeft().v,
                             core_.yMin() + cell->getBottom().v);
  db_inst->setLocation(final_loc.x(), final_loc.y());

  debugPrint(logger_,
             DPL,
             "place",
             1,
             "placeAt {} requested ({},{}) -> placed ({},{})",
             db_inst->getName(),
             requested_loc.x(),
             requested_loc.y(),
             final_loc.x(),
...
std::optional<odb::Point> Opendp::findLegalLocation(
    odb::dbInst* db_inst,
    const odb::Point& requested_loc)
{
  Node* cell = network_->getNode(db_inst);
  if (cell == nullptr || cell->isFixed() || !cell->isPlaced()) {
    return std::nullopt;
  }
  ...
  // Refuse the dry-run if the current location can't be restored. See the
  // matching guard in placeAt() above: the restore path calls
  // placeCell(orig_x, orig_y) which would crash on getSiteOrientation().value()
  // when row_sites_ has no entry for the cell's master site at orig_y.
  odb::dbSite* site = cell->getDbInst()->getMaster()->getSite();
  if (!grid_->getSiteOrientation(orig_x, orig_y, site)) {
    return std::nullopt;
  }
  ...
  std::optional<odb::Point> result;
  if (found.pixel != nullptr) {
    // Compute what the dbu location would be if we committed found.
    // Mirror placeCell's coordinate math without actually painting the grid.
    const DbuX site_w = grid_->getSiteWidth();
    const odb::Point candidate(core_.xMin() + gridToDbu(found.x, site_w).v,
                               core_.yMin() + grid_->gridYToDbu(found.y).v);
    result = candidate;
  }

  // Always restore. The original pixel was just freed and nothing else can
  // claim it in the single-threaded dpl context, so re-placing at orig is
